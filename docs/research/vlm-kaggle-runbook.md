...
1. This branch (`research/vlm-kaggle-prep`) pushed to a GitHub remote. The runner records `git rev-parse HEAD`, so run from a clean clone of a pushed commit.
...
```bash
cd /kaggle/working
git clone <PUSHED_REPO_URL> satquery && cd satquery
git checkout <PUSHED_COMMIT_SHA> && git rev-parse HEAD
