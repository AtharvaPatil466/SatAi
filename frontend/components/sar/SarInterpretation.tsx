...
export function SarInterpretation({ report }: { report: SarReport }) {
  return (
    <div className="space-y-5">
      ...
      <details className="panel group p-5"><summary className="cursor-pointer list-none text-sm font-semibold text-accent">View full analyst annotation <span className="ml-1 group-open:hidden">+</span><span className="ml-1 hidden group-open:inline">−</span></summary><div className="prose-annotation mt-5 whitespace-pre-wrap border-t border-border pt-4 text-sm leading-6 text-slate-300">{report.annotation}</div><p className="mt-4 border-t border-border pt-3 text-[11px] leading-relaxed text-slate-500">Verbatim from the committed analyst annotation (data/sar_gate/annotation_template.md), including the analyst's own confidence and uncertainty notes. Every interpretation on this page is human-produced.</p></details>
    </div>
  );
}
